...
abstract: The `permimp`-package is developed to replace the conditional permutation importance computation by the `varimp`-function(s) of the `party`-package. `permimp` does everything that the `varimp`-function(s) do, but (for the conditional permutation) it is faster. In addition the package includes some plotting options.   
keywords: "permimp, permutation importance, cforest"
date: "`r Sys.Date()`"
bibliography: G:/Forschung/VariableImportance/RF_VariableImportance.bib
biblio-styke: apa
---
...
# Introduction
Although originally designed for prediction purposes, Random forests @Breiman2001 have become a popular tool to assess the importance of predictors. Several methods and measures have been proposed, one of the most popular ones is the *permuation importance* @Breiman2001. Inspired by the contrast between the *unconditional* zero-order correlation between predictor and outcome, and the *conditional* standardized regression coefficient in multiple linear regression, @Strobl2008 argued that in some cases the importance of a predictor, conditionally on (all) other predictors, may be of higher interest than the *unconditional* importance. Therefore, they proposed the *conditional permuation importance*, which introduces a conditional permutation scheme that is based on the dependence between the predictors.

The `permimp`-package presents a different implementation of this *conditional permutation importance*. Like the original implementation (available in the `party` R-package of  @Hothorn2006), `permimp` focuses on random forests that were grown according to the unbiased recursive partitioning (cf. `cforests`; @Hothorn2006). Moreover, the `permimp`-package can be seen as a replacement for the `varimp`-functions of the `party` package in R. 

This vignette has two main parts. The first part is tutorial-like and demonstrates functionality of the `permimp`-package (by also comparing it to original `party::varimp`-functions. The second part is more theoretical and explains the how and the why of the new *conditional permuation importance*-implementation.
...
The `permimp`-function replaces all the `party::varimp`-functions (`varimp`, `varimpAUC`, `varimpsurv`). To apply `permimp`-function, one needs a fitted random forest. More specifically, a S4 object of class `"RandomForest"` as obtained by the `party::cforest`-function. As an example we will use the (cleaned) `airquality`-data set to fit random forest with 50 trees:
...
### A.1. New Conditional Permuation Importance

Let's start by comparing the `permimp` and the `varimp` function for the conditional permutation importance.

```{r eval = TRUE, echo = TRUE} 
system.time(Cpermimp <- permimp(cfAirq50, conditional = TRUE, progressBar = FALSE))
system.time(Cvarimp <- varimp(cfAirq50, conditional = TRUE))
Cpermimp
Cvarimp
```
...
* `perminp` has a `progressBar`-argument. The default is `progressBar = TRUE`^[`progressBar = FALSE` was used, because a progress bar is not represented well in Rmarkdown.]
...
There are two main reasons. First, `permimp` uses a different default `threshold`-value:  `permimp` uses `threshold = .95` while `varimp` uses `threshold = 0.2`. Check `?permimp` and `?varimp`.

When using equal `threshold`-values...  

```{r eval = TRUE, echo = TRUE} 
Cpermimp <- permimp(cfAirq50, conditional = TRUE, threshold = .2, progressBar = FALSE)
Cpermimp
Cvarimp
```
...
```{r eval = TRUE, echo = TRUE} 
Cvarimp <- varimp(cfAirq50, conditional = TRUE, threshold = .95)
Cpermimp
Cvarimp
```

The differences between the two implementations (and why we believe the new implementation is more attractive), is explained in the second part of this document. 

### A.3. Backward Compatibel with `party` when: `asParty = TRUE`

By specifying `asParty = TRUE`, the `permimp`-function can be made backward compatible with the `party::varimp`-function.  But `permimp` is a bit faster. To get exactly the same results, the used random seeds should be equa. 

```{r eval = TRUE, echo = TRUE} 
set.seed(542863)
system.time(CasParty <- permimp(cfAirq50, conditional = TRUE, asParty = TRUE, progressBar = FALSE))
set.seed(542863)
system.time(Cvarimp <- varimp(cfAirq50, conditional = TRUE))
CasParty
Cvarimp
...
1. `$values`: holds the computed variable importances. 
2. `$perTree`: holds the variable importances per tree (averaged over the permutations when `nperm > 1`).
...
all(CasParty$values == Cvarimp)
```

An advantage of the `VarImp`-object, is that the `$perTree`-values kan be used to inspect the distribution of the importance values across the trees in a forest. For instance, the plotting function (demonstrated below) can be used to visualize this distribution of per tree importance values.


### A.5. Unconditional Permutation Importance: `permimp` = `varimp`

Of course, there is also the option to compute the unconditional permutation importance. Both using the original and the splitwise permutation. Here, there are no differences between `permimp` and `varimp`. That is, `permimp` simply uses the `party` `varimp` code, making the `asParty` argument redundant in this case. Note, however, that `permimp` still returns a `VarImp-object`.

```{r eval = TRUE, echo = TRUE} 
## Original Unconditional Permutation Importance
set.seed(542863)
UasParty <- permimp(cfAirq50, progressBar = FALSE, pre1.0_0 = TRUE)
set.seed(542863)
Uvarimp <- varimp(cfAirq50, pre1.0_0 = TRUE)
UasParty
Uvarimp

## Splitwise Unconditional Permutation Importance
set.seed(542863)
UasParty <- permimp(cfAirq50, progressBar = FALSE)
set.seed(542863)
Uvarimp <- varimp(cfAirq50)
UasParty
Uvarimp
```
...
Visualizing the variable importances (as a `VarImp`-object) is easy using the `plot` method. Its main features include:
...
Upermimp <- permimp(cfAirq500, progressBar = FALSE)

## different plots, all easy to make
## barplot
plot(Upermimp, type = "bar")
...
plot(Upermimp, type = "bar", interval = "quantile")

## horizontal boxplot
plot(Upermimp, type = "box", horizontal = TRUE)

## unsorted dotplot
plot(Upermimp, type = "dot", sort = FALSE, 
     interval = "quantile")
```
...
# Part II: New Conditional Permutation Implentation
...
## A. Recapitulation: Conditional Permuation Importance.
...
@Strobl2008 proposed to specify a partitioning (grid) of the predictor space based on $Z$ (for each tree), in order to (conditionally) permute the values of $X$ withing each parition (i.e., cell in the grid). According to @Strobl2008 this partitioning should (1) be applicable to variables of all types; (2) be as parsimonious as possible, but (3) be also computationally feasible. Therefore they suggested to define the partitioning grid for each tree by means of the partitions of the predictor space induced by that tree. More precisely, using all the splitpoints for $Z$ in the tree, $Z$ is discritized and the complete predictor space is partitioned using the discritized $Z$. 

Note that this paritioning does not correspond with the recursive partitioning of a tree. In a tree only the top node splits the complete predictor space, all the following splits are conditional on the parent nodes. In contrast, for the conditional permuation grid, all the split points split the complete predictor space, which leads to a more fine-grained grid.


In practice, the number of observations is finite. In situation with a relatively low number of observations, the grid for the conditional permuation may become to fine grained, making conditionally permuting practically infeasible. Therefore, the selection of $Z$ (the predictors to condition on) is not a sinecure.

## B. Original Implementation (`party::varimp`)
In their original implementation (cf, `party::varimp`), @Strobl2008 argued to only include those variables in $Z$ whose empirical correlation with $X$ exceeds a certain moderate threshold. For contiuous variables the Pearson correlation could be used, but for the general case they proposed to use the conditional inference framework promoted by @Hothorn2006. Applying this framework provides *p*-values, which have the advantage that they are comparable for variables of all types, and that they can serve as an intuitive and objective means of selecting the variables Z to condition on.

The original implementation can be described as follows:

> For every predictor $X$
> 
> 1. Test which other predictors are related to $X$, aplying the conditional inference framework [@Hothorn2006Lego] using **the full data/training set**.
> 2. Only inlcude those other predictors in $Z$ for which the $p$-value of the test is smaller than `(1 - threshold)`.
> 3. Within each tree:
>     1. Gather all the splitpoints for every predictor in $Z$.
>     2. Discritize the predictors in $Z$ using the gathered splitpoints, and create a partitioning of the predictor space.
>     3. Within each partition, permute the values of predictor $X$.
   
    
### Some issues
There are, however, two important issutes with this implementation:

1. For (some of) the tests in the conditional inference framework, the $p$-values not only depen\ on the strenght of the (cor)relation, but also on the sample size. For instance, in big samples small correlations can also lead to small $p$-values.
...
> 
> 1. Discretize the in-bag values for each predictor using the splitpoints: $X$ => $X_d$.
> 
> 2. For every discretized $X_d$:
> 
>     1. Test which other discretized predictors $W_d$ are related to $X_d$, aplying a $\chi^2$-independence tests (using only the in-bag values).
>     2. Only inlcude those other predictors $W$ in $Z$ for which the $p$-value of the test is smaller than `(1 - threshold)`.
>     3. Create the partitioning of the predictor space uzing the discretized $Z$.
...
The `threshold`-value can be interpreted as a tuning parameter to make the permuation more or less conditional. A `threshold = 0` and a `threshold = 1` corresponding to permuatationg as conditional as possible and permutating completely unconditional, respectively. A `threshold = .95`, the default in `permimp`, only includes those $W$ in $Z$ for which $W_d$ and $X_d$ are dependent (with $\alpha$-level = .05).  

Some research questions are best answered with an unconditional importance, while other questions are better answered using a conditional importance measure. In many situations, however, it is not clear which measure best fits the research question. Therefore, we argue that in these cases it can be interesting to evaluate the importance (rankings) of the predictors for different `threshold`-values, going from fully unconditional to more conditional. This strategy can provide more insight in how the conditioning affects the permutation importance values.

In the original implementation, setting a sensible `threshold` proved to be hard, because the practial meaning of the `threshold` depended on the sample size and on the type of variables (cf. the issues raised above). In the new implementation, the `threshold`'s interpretation is clearer and more stable. In addition, the simulation studies by Debeer and Strobl (2018) ADD CITATION suggest that the new implementation (a) allows a more gradual shift from unconditional to conditional; and (b) gives more stable importance measure computations for lower `threshold`-values.

As an additional feature, the `permimp` can provide some diagnostics about the conditional permuation. When  `thresholdDiagnostics = TRUE`, the `permimp`-function monitors whether or not a conditional permuation scheme was feasible for each predictor $X$ in each tree. This information is translated in messages that suggest to either  or decrease the `threshold`. 

First, it is possible that the conditioning grid is so fine-grained that permuating $X$ conditionally cannot lead to observations ending up in a different end-node of the tree. In other words, the prediction accuracy before and after permuting will be always equal. If this issue occurs in more than 50 percent of the trees that include $X$ as a splitting variable, `permimp` will produce a note, and suggest to increase the `threshold`-value. A higher `threshold`-value may result in a less fine-grained partitioning, making the conditional permutation feasible again.
