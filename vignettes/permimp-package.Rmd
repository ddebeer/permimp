...
The `threshold`-value can be interpreted as a tuning parameter to make the permuation more or less conditional. A `threshold = 0` and a `threshold = 1` corresponding to permutating as conditional as possible and permutating completely unconditional, respectively. A `threshold = .95`, the default in `permimp`, only includes those $W$ in $Z$ for which $W_d$ and $X_d$ are dependent (with $\alpha$-level = .05). 

Yet threshold values smaller than `threshold = .5` generally make the selection of the predictors to condition on too greedy, without a meaningfull impact on the CPI pattern. Therefore, we recommend using threshold values between .5 and 1.  
